...
fn test_error_invalid_count_zero() {
    ...
    let output = run_rfamily(&["--preset", "english", "--count", "0", "--output", output_file]);
...
fn test_error_invalid_count_negative() {
    ...
    let output = run_rfamily(&["--preset", "english", "--count", "-10", "--output", output_file]);
...
fn test_error_invalid_count_non_numeric() {
    ...
    let output = run_rfamily(&["--preset", "english", "--count", "abc", "--output", output_file]);
    
    assert!(!output.status.success(), "Should fail with non-numeric count");
...
fn test_error_preset_not_found() {
    ...
    let output = run_rfamily(&["--preset", "klingon", "--count", "100", "--output", output_file]);
    
    assert!(!output.status.success(), "Should fail with unknown preset");
    
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("klingon") || stderr.contains("not found") || stderr.contains("Unknown"), 
...
fn test_error_empty_preset_name() {
    ...
    assert!(!output.status.success(), "Should fail with empty preset name");
...
fn test_error_case_sensitive_preset() {
    ...
    let output = run_rfamily(&["--preset", "ENGLISH", "--count", "100", "--output", output_file]);
    
    assert!(!output.status.success(), "Should fail with uppercase preset name (case sensitive)");
...
fn test_error_malformed_ruleset_json() {
    ...
    let output = run_rfamily(&["--ruleset", ruleset_file, "--count", "100", "--output", output_file]);
    ...
    if !output.status.success() {
        assert!(combined.contains("parse") || combined.contains("JSON") || combined.contains("error") || combined.contains("Error"),
...
fn test_error_incomplete_ruleset_json() {
    ...
    assert!(!output.status.success(), "Should fail with incomplete ruleset");
...
fn test_error_nonexistent_ruleset_file() {
    ...
    let output = run_rfamily(&["--ruleset", "nonexistent.json", "--count", "100", "--output", output_file]);
    
    assert!(!output.status.success(), "Should fail with nonexistent ruleset file");
    
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("nonexistent.json") || stderr.contains("No such file") || stderr.contains("not found"),
...
fn test_error_write_to_readonly_directory() {
    ...
    if !output.status.success() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        let combined = format!("{}{}", stderr, stdout);
        assert!(combined.contains("Permission") || combined.contains("denied") || combined.contains("error") || combined.contains("Error"),
...
fn test_error_conflicting_preset_and_ruleset() {
    ...
    let output = run_rfamily(&["--preset", "spanish", "--ruleset", ruleset_file, "--count", "50", "--output", output_file]);
    
    // This should work - ruleset takes precedence
    assert!(output.status.success(), "Should succeed with both flags (ruleset takes precedence)");
...
fn test_error_no_preset_no_ruleset() {
    ...
    assert!(output.status.success(), "Should succeed with default preset");
...
fn test_error_special_characters_in_output_filename() {
    ...
    let output = run_rfamily(&["--preset", "english", "--count", "50", "--output", output_file]);
    ...
    assert!(output.status.code().is_some(), "Should exit with a status code");
...
fn test_error_very_long_filename() {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        assert!(stderr.contains("too long") || stderr.contains("name") || stderr.contains("error"),
...
fn test_error_output_file_already_exists_overwrite() {
    ...
    assert!(output.status.success(), "Should succeed and overwrite existing file");
    ...
    assert!(!content.contains("existing content"), "Should have overwritten old content");
...
fn test_error_multiple_deprecated_flags() {
    ...
    let output = run_rfamily(&["--lds", "--spanish", "--count", "50", "--output", output_file]);
    
    assert!(output.status.success(), "Should succeed with multiple deprecated flags");
    
    // Should show warnings for deprecated flags
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("deprecated") || stderr.contains("Deprecated"),
...
fn test_error_generate_ruleset_invalid_path() {
    ...
    assert!(!output.status.success(), "Should fail with invalid directory path");
    
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("No such file") || stderr.contains("directory") || stderr.contains("error"),
...
fn test_error_generate_ruleset_overwrite() {
    ...
    assert!(output2.status.success(), "Should succeed overwriting ruleset");
...
fn test_error_extremely_large_count() {
    ...
    let output = run_rfamily(&["--preset", "english", "--count", "10000", "--output", output_file]);
...
fn test_error_utf8_in_filename() {
    ...
    let output = run_rfamily(&["--preset", "japanese", "--count", "30", "--output", output_file]);
...
fn test_error_whitespace_in_preset_name() {
    ...
    assert!(!output.status.success(), "Should fail with whitespace in preset name");
...
fn test_error_list_presets_exits_without_generating() {
    ...
    assert!(!Path::new("output.ged").exists(), "Should not create default output file when listing presets");
}
...
fn test_error_generate_ruleset_exits_without_generating() {
    ...
    assert!(Path::new(ruleset_file).exists(), "Ruleset file should be created");
    
    // Should not create GEDCOM output file when just generating ruleset
    assert!(!Path::new("output.ged").exists(), "Should not create default output file when generating ruleset");
...
fn test_error_empty_output_filename() {
    ...
    assert!(!output.status.success(), "Should fail with empty output filename");
}
...
fn test_error_output_to_directory_not_file() {
    ...
    assert!(!output.status.success(), "Should fail writing to directory instead of file");
    
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("directory") || stderr.contains("Is a directory") || stderr.contains("error"),
