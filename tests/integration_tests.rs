...
fn count_individuals(content: &str) -> usize {
    content.lines().filter(|line| line.starts_with("0 @I")).count()
}

/// Helper to count families in GEDCOM file
fn count_families(content: &str) -> usize {
    content.lines().filter(|line| line.starts_with("0 @F")).count()
}
...
fn test_cli_list_presets() {
    ...
    assert!(line_count >= 52, "Should list all 51+ presets, got {}", line_count);
}
...
fn test_cli_generate_with_preset() {
    ...
    assert!(individual_count >= 100, "Should generate at least 100 individuals, got {}", individual_count);
    assert!(individual_count <= 120, "Should not generate too many individuals, got {}", individual_count);
    
    // Check for English-specific content
    assert!(content.contains("1 CHAR UTF-8"), "Should use UTF-8 encoding");
...
fn test_cli_generate_with_japanese_preset() {
    ...
    let output = run_rfamily(&["--preset", "japanese", "--count", "50", "--output", output_file]);
    ...
    let individual_count = count_individuals(&content);
    assert!(individual_count >= 50, "Should generate at least 50 individuals, got {}", individual_count);
    assert!(individual_count <= 65, "Should not generate too many individuals, got {}", individual_count);
...
fn test_cli_generate_with_arabic_preset() {
    ...
    let output = run_rfamily(&["--preset", "arabic", "--count", "50", "--output", output_file]);
...
fn test_cli_deprecated_lds_flag() {
    ...
    assert!(content.contains("BAPL") || content.contains("CONF") || content.contains("ENDL"), 
...
fn test_cli_deprecated_spanish_flag() {
    ...
    let individual_count = count_individuals(&content);
    assert!(individual_count >= 75, "Should generate at least 75 individuals, got {}", individual_count);
    assert!(individual_count <= 100, "Should not generate too many individuals, got {}", individual_count);
...
fn test_cli_short_flags() {
    ...
    let individual_count = count_individuals(&content);
    assert!(individual_count >= 60, "Should generate at least 60 individuals, got {}", individual_count);
    assert!(individual_count <= 80, "Should not generate too many individuals, got {}", individual_count);
...
fn test_cli_generate_ruleset() {
    ...
    let parsed: serde_json::Value = serde_json::from_str(&content)
        .expect("Generated ruleset should be valid JSON");
    
    // Check for expected fields in names object
    let names = parsed.get("names").expect("Should have names object");
    assert!(names.get("male_given_names").is_some(), "Should have male_given_names");
    assert!(names.get("female_given_names").is_some(), "Should have female_given_names");
...
fn test_cli_with_custom_ruleset() {
    ...
    let output = run_rfamily(&["--ruleset", ruleset_file, "--count", "80", "--output", output_file]);
    ...
    let individual_count = count_individuals(&content);
    assert!(individual_count >= 80, "Should generate at least 80 individuals, got {}", individual_count);
    assert!(individual_count <= 100, "Should not generate too many individuals, got {}", individual_count);
...
fn test_cli_family_relationships() {
    ...
    let output = run_rfamily(&["--preset", "english", "--count", "200", "--output", output_file]);
...
fn test_cli_invalid_preset_name() {
    ...
    let output = run_rfamily(&["--preset", "nonexistent_preset", "--count", "100", "--output", output_file]);
    
    // Should fail with error
    assert!(!output.status.success(), "Command should fail with invalid preset");
    
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("Unknown preset") || stderr.contains("not found"), 
...
fn test_cli_utf8_encoding_consistency() {
    ...
    for preset in &["chinese", "thai", "arabic", "russian", "korean"] {
        ...
        assert!(output.status.success(), "Command should succeed for {} preset", preset);
        
        let (valid, content) = verify_gedcom_file(output_file);
        assert!(valid, "Generated file should be valid GEDCOM for {} preset", preset);
        
        // All should use UTF-8
        assert!(content.contains("1 CHAR UTF-8"), 
                "{} preset should use UTF-8 encoding", preset);
...
fn test_cli_large_generation() {
    ...
    let output = run_rfamily(&["--preset", "english", "--count", "1000", "--output", output_file]);
    
    assert!(output.status.success(), "Command should succeed for large generation");
    
    let (valid, content) = verify_gedcom_file(output_file);
    assert!(valid, "Generated file should be valid GEDCOM");
    
    let individual_count = count_individuals(&content);
    assert!(individual_count >= 1000, "Should generate at least 1000 individuals, got {}", individual_count);
    assert!(individual_count <= 1050, "Should not generate too many individuals, got {}", individual_count);
    
    // Check file size is reasonable (should be multiple KB)
    let metadata = fs::metadata(output_file).expect("Should get file metadata");
    assert!(metadata.len() > 10000, "File should be at least 10KB for 1000 individuals");
...
fn test_cli_all_asian_presets() {
    ...
    let asian_presets = vec![
        "chinese", "japanese", "khmer", "korean", "mongolian", "thai", "vietnamese"
    ];
