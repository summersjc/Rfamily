...
impl PresetRegistry {
    pub fn new() -> Self {
        ...
        presets.insert(
            "british".to_string(),
            include_str!("../presets/british.json"),
        );
        presets.insert(
            "croatian".to_string(),
...
mod tests {
    ...
    fn test_registry_loads_all_presets() {
        ...
        assert!(
            presets.len() >= 52,
            "Expected at least 52 presets, found {}",
            presets.len()
        );
    ...
    #[test]
    fn test_preset_count_is_exactly_52() {
        let registry = PresetRegistry::new();
        let presets = registry.list();
        ...
        assert_eq!(
            presets.len(),
            52,
            "Expected exactly 52 presets, found {}",
            presets.len()
        );
    }
    ...
    fn test_regional_grouping_totals() {
        ...
        assert_eq!(total, 52, "Regional grouping should total 52 presets");
        assert_eq!(registry.list().len(), 52, "Registry should have 52 presets");

        // Verify counts
        assert_eq!(european_count, 31, "Should have 31 European presets");
